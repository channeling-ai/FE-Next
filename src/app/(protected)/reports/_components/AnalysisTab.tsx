...
function RetentionGraph({
    ...
    const criticalStartX = toX(parseTimeToSeconds(criticalStartTime))
    const criticalEndX = toX(parseTimeToSeconds(criticalEndTime))
        ...
        <div className="w-full overflow-x-auto overflow-y-hidden pt-4">
            <div className="min-w-[292px]">
                ...
                >
                    ...
                    {[criticalStartX, criticalEndX].map((x, index) => (
                        <line
                            key={`${x}-${index}`}
                            stroke="#E9495A"
                            strokeDasharray="2 2"
...
function AnalysisTabContent({ analysis }: { analysis: ReportAnalysis }) {
    ...
    const { criticalSection, causes, improvements, expectedEffect } = viewerRetentionAnalysis
        ...
        <div className="-ml-0.5 flex w-[calc(100%+8px)] flex-col gap-8 pt-4 tablet:ml-0 tablet:w-full">
                ...
                <div className="flex flex-col gap-4 overflow-hidden rounded-[20px] bg-bg-1 p-5">
                    <div className="flex flex-col gap-1">
                        <p className="font-body-14m text-text-brand">
                            {formatKoreanTime(criticalSection.startTime)}({criticalSection.startTime}~
                            {criticalSection.endTime}) 구간 이탈 요약
                        </p>
                    ...
                    {retentionGraph.length > 0 && (
                        <RetentionGraph
                            points={retentionGraph}
                            criticalStartTime={criticalSection.startTime}
                            criticalEndTime={criticalSection.endTime}
                        />
                    )}
            ...
            <section className="flex flex-col gap-2" aria-labelledby="algorithm-optimization-title">
                ...
                <div className="flex flex-col gap-4 rounded-[20px] bg-bg-1 p-5">
                    {algorithmOptimization.categoryList.map((item, index) => {
                                ...
                                <AnalysisItem
                                    label={
                                        <span className="flex items-center justify-between gap-2">
                                            ...
                                            <ScoreBadge score={item.score} status={grade.label} tone={grade.tone} />
                                        </span>
                                    }
                                >
                    ...
                    {algorithmOptimization.additionalSuggestions.length > 0 && (
                        <>
                            <Divider />
                            <AnalysisItem label={`${algorithmOptimization.categoryList.length + 1}. 추가 제안`}>
                                {algorithmOptimization.additionalSuggestions.map((suggestion, index) => (
